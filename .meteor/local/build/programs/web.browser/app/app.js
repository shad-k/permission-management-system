...
// Template.adminPanel.helpers({                                                                             // 9
//   users() {                                                                                               // 10
//     return Meteor.users.find({});                                                                         // 11
//   },                                                                                                      // 12
// });                                                                                                       // 13
Template.adminPanel.events({                                                                                 // 15
	'submit .createUser': function (event) {                                                                    // 16
		event.preventDefault();                                                                                    // 17
		var permission = event.target.permission.value;                                                            // 19
		var username = event.target.username.value;                                                                // 21
		var password = event.target.password.value;                                                                // 23
                                                                                                             //
		if (username && password) {                                                                                // 25
			Meteor.call('accounts.addUser', username, password, function (error, result) {                            // 26
				Meteor.call('roles.setRole', result, permission);                                                        // 26
			});                                                                                                       // 26
		}                                                                                                          // 27
                                                                                                             //
		event.target.username.value = "";                                                                          // 30
		event.target.password.value = "";                                                                          // 31
	}                                                                                                           // 32
});                                                                                                          // 15
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
Template.allUsers.events({                                                                                   // 18
	'click .blockUser': function (event) {                                                                      // 19
		Meteor.call('roles.blockUser', this._id);                                                                  // 20
	},                                                                                                          // 21
	'click .unblockUser': function (event) {                                                                    // 22
		Meteor.call('roles.unblockUser', this._id);                                                                // 23
	},                                                                                                          // 24
	'click .changeProfileInfo': function (event) {                                                              // 25
		document.querySelector(".changeProfileDiv").style.display = "block";                                       // 26
		var user = Meteor.users.findOne({                                                                          // 27
			_id: this._id                                                                                             // 27
		});                                                                                                        // 27
		document.querySelector(".un").value = user.username;                                                       // 28
                                                                                                             //
		if (user.username === "root") {                                                                            // 29
			document.querySelector(".name").value = "admin details can't be shown";                                   // 30
			document.querySelector(".email").value = "admin details can't be shown";                                  // 31
			document.querySelector(".phone").value = "admin details can't be shown";                                  // 32
		} else {                                                                                                   // 33
	...
	'click .removeUser': function (event) {                                                                     // 51
		Meteor.call('user.removeUser', this._id);                                                                  // 52
	},                                                                                                          // 53
	'click .updateUser': function (event) {                                                                     // 54
		var name = document.querySelector(".name").value;                                                          // 55
		var email = document.querySelector(".email").value;                                                        // 56
		var phone = document.querySelector(".phone").value;                                                        // 57
		var username = document.querySelector(".un").value;                                                        // 59
		Meteor.call('user.updateUser', username, name, email, phone, function (error, result) {                    // 61
			if (error) {                                                                                              // 62
				alert(error);                                                                                            // 63
			} else if (result === true) {                                                                             // 64
				alert("User profile updated");                                                                           // 66
			} else {                                                                                                  // 67
				alert(result);                                                                                           // 69
			}                                                                                                         // 70
		});                                                                                                        // 71
...
Template.body.onCreated(function () {                                                                        // 16
	function bodyOnCreated() {                                                                                  // 16
		Meteor.call('setInitialRole');                                                                             // 17
	}                                                                                                           // 18
                                                                                                             //
	return bodyOnCreated;                                                                                       // 16
}());                                                                                                        // 16
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
// import { Roles } from 'meteor/alanning:roles';                                                            // 7
Template.changeProfileInfo.helpers({                                                                         // 10
	profile: function () {                                                                                      // 11
		return Meteor.users.find({                                                                                 // 12
			_id: this._id                                                                                             // 12
		});                                                                                                        // 12
	}                                                                                                           // 13
});                                                                                                          // 10
...
Template.dashboard.events({                                                                                  // 17
	'click .logout': function (event) {                                                                         // 18
		Meteor.logout();                                                                                           // 19
	}                                                                                                           // 20
});                                                                                                          // 17
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
Meteor.methods({                                                                                             // 10
	'messages.insert': function (text) {                                                                        // 11
		check(text, String); // Make sure the user is logged in before inserting a message                         // 12
                                                                                                             //
		if (!Meteor.userId()) {                                                                                    // 15
